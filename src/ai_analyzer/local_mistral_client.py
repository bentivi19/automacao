...
class LocalMistralAnalyzer:
    def __init__(self):
        model_path = os.path.join("C:", "Users", "Julio Soama", "Desktop", "mistral", "mistral-7b-instruct-v0.2.Q4_K_M.gguf")
        if not os.path.exists(model_path):
            raise ValueError(f"Modelo não encontrado em: {model_path}")
        
        # Inicializa o modelo com configurações otimizadas
        self.llm = Llama(
            model_path=model_path,
            n_ctx=8192,  # Contexto máximo
            n_batch=512,  # Tamanho do batch para inferência
            n_threads=8,  # Número de threads para processamento
            n_gpu_layers=35  # Número de camadas para rodar na GPU
        )
        
        self.knowledge_base = LegalKnowledgeBase()
        self.pdf_splitter = PDFSplitter(max_tokens_per_chunk=4000)  # Reduzido para o Mistral local
        self.load_rules()
    ...
    def process_document(self, file_path: str) -> Dict:
        ...
        try:
            ...
            # Divide o PDF em chunks menores
            ...
            # Garante que o PDFReader seja fechado
    ...
    def _analyze_chunk(self, text: str, is_last_chunk: bool = False) -> str:
        ...
        try:
            prompt = f"""[INST]Você é um assistente especializado em análise de documentos jurídicos.
                     ...
                     Analise este texto seguindo as instruções:
    ...
    def _consolidate_analyses(self, analyses: List[str]) -> str:
        ...
        try:
            ...
            prompt = f"""[INST]Você é um assistente especializado em análise de documentos jurídicos.
                     ...
                     Consolide estas análises seguindo as regras especificadas:

                     {consolidated_input}[/INST]"""
    ...
    def _generate_recommendations(self, analysis: str) -> List[str]:
        ...
        try:
            prompt = f"""[INST]Gere recomendações práticas baseadas na análise do documento.

                     Com base nesta análise, quais são as recomendações principais?
